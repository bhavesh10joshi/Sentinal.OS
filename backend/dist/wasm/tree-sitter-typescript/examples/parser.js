...
(function (ts) {
    ...
    (function (Parser) {
        ...
        function createQualifiedName(entity, name) {
            const node = createNode(SyntaxKind.QualifiedName, entity.pos);
            node.left = entity;
            node.right = name;
            return finishNode(node);
        }
        ...
        function parseThisTypePredicate(lhs) {
            nextToken();
            const node = createNode(SyntaxKind.TypePredicate, lhs.pos);
        ...
        function parseJSDocAllType() {
            const result = createNode(SyntaxKind.JSDocAllType);
            nextToken();
            return finishNode(result);
        }
        ...
        function parseLiteralTypeNode(negative) {
            const node = createNode(SyntaxKind.LiteralType);
            let unaryMinusExpression;
            if (negative) {
                unaryMinusExpression = createNode(SyntaxKind.PrefixUnaryExpression);
                unaryMinusExpression.operator = SyntaxKind.MinusToken;
                nextToken();
            }
            let expression = token() === SyntaxKind.TrueKeyword || token() === SyntaxKind.FalseKeyword
                ? parseTokenNode()
                ...
                switch (token()) {
                    ...
                    case SyntaxKind.OpenBracketToken:
                        ...
                        if (isStartOfType()) {
                            ...
                            const node = createNode(SyntaxKind.ArrayType, type.pos);
        ...
        function parseTypeOperatorOrHigher() {
            ...
            switch (operator) {
                ...
                case SyntaxKind.DotDotDotToken: {
                    const result = createNode(SyntaxKind.JSDocVariadicType);
                    nextToken();
        ...
        function parseJsxOpeningOrSelfClosingElementOrOpeningFragment(inExpressionContext) {
            ...
            if (token() === SyntaxKind.GreaterThanToken) {
                ...
                const node = createNode(SyntaxKind.JsxOpeningFragment, fullStart);
                return finishNode(node);
            }
        ...
        function parseJsxElementName() {
            ...
            let expression = token() === SyntaxKind.ThisKeyword ?
        ...
        function parseObjectLiteralElement() {
            ...
            const isShorthandPropertyAssignment = tokenIsIdentifier && (token() === SyntaxKind.CommaToken || token() === SyntaxKind.CloseBraceToken || token() === SyntaxKind.EqualsToken);
        ...
        function parseNamedImportsOrExports(kind) {
            ...
            node.elements = parseBracketedList(ParsingContext.ImportOrExportSpecifiers, kind === SyntaxKind.NamedImports ? parseImportSpecifier : parseExportSpecifier, SyntaxKind.OpenBraceToken, SyntaxKind.CloseBraceToken);
            return finishNode(node);
        }
        ...
        function processReferenceComments(sourceFile) {
            ...
            let amdModuleName;
            let checkJsDirective = undefined;
        ...
        function setExternalModuleIndicator(sourceFile) {
            sourceFile.externalModuleIndicator = forEach(sourceFile.statements, node => hasModifier(node, ModifierFlags.Export)
        ...
        (function (JSDocParser) {
            ...
            function parseJSDocCommentWorker(start, length) {
                ...
                function parsePropertyAccessEntityNameExpression() {
                    ...
                    while (parseOptional(SyntaxKind.DotToken)) {
                        const prop = createNode(SyntaxKind.PropertyAccessExpression, node.pos);
                    ...
                    if (!typeExpression || isObjectOrObjectArrayTypeReference(typeExpression.type)) {
                        ...
                        while (child = tryParse(() => parseChildParameterOrPropertyTag(PropertyLikeParse.Property))) {
                            ...
                            if (child.kind === SyntaxKind.JSDocTypeTag) {
                                ...
                                jsdocTypeLiteral.jsDocPropertyTags = append(jsdocTypeLiteral.jsDocPropertyTags, child);
                            }
                        }
                        if (jsdocTypeLiteral) {
